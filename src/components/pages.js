...
export default class Pages extends React.Component{
    ...
    update(e){
		...
		this.state.design = Number(e.target.value)
		this.subtotal()
	}
	pageNumber(e){
		e.preventDefault()
		
		this.state.pages = Number(e.target.value)
		this.subtotal()
	}
	subtotal(){
		this.state.subtotal = this.state.design + (this.state.pages * 100);
		...
		return (
			...
			<form className="d-flex flex-column flex-wrap flex-row justify-content-center align-items-around">
				...
				<div className="my-4 col-12 d-flex flex-row align-items-center justify-content-start">
					...
					<input className="pl-2" onChange={this.pageNumber} placeholder="0" id="pages" min="1" max="1000" step="1" type="number" required/>
				</div>
